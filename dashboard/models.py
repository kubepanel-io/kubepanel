...
class Package(models.Model):
    ...
    max_domains = models.IntegerField(null=True, blank=True, help_text="Maximum domains per user (null = unlimited)")
...
class Domain(models.Model):
    ...
    def clean(self):
        ...
        # Check domain count limit (for new domains only)
        if not self.pk and pkg.max_domains is not None:
            current_domain_count = domains.count()
            if current_domain_count >= pkg.max_domains:
                raise ValidationError({
                    '__all__': f"Domain limit reached ({current_domain_count}/{pkg.max_domains}). "
                               f"Your package allows a maximum of {pkg.max_domains} domains."
                })
...
class DomainAlias(models.Model):
    ...
    dkim_selector = models.CharField(max_length=63, blank=True, default='default')
    dkim_secret_name = models.CharField(max_length=253, blank=True, default='')
...
class MailUser(models.Model):
    ...
    domain_alias = models.ForeignKey(
        DomainAlias, on_delete=models.CASCADE,
        null=True, blank=True, related_name='mail_users',
    )
    ...
    def mail_domain_name(self):
        if self.domain_alias:
            return self.domain_alias.alias_name
        return self.domain.domain_name

    @property
    def email(self):
        return f"{self.local_part}@{self.mail_domain_name}"
...
Immediate action required to prevent service disruption.""",
    ...
    # Site Branding (superuser only)
    site_name = models.CharField(
        max_length=100,
        default='Business Suite',
        help_text='Displayed under the logo in the sidebar'
    )
    site_footer = models.CharField(
        max_length=200,
        default='KubePanel – kubepanel.io – Example Company Ltd',
        help_text='Displayed at the bottom of the sidebar'
    )

    # Feature flags
    cpanel_migration_enabled = models.BooleanField(
        default=False,
        help_text='Enable cPanel Migration tool in admin menu'
    )
...
# =============================================================================
# cPanel Migration Models
# =============================================================================

class MigrationBatch(models.Model):
    """
    A batch migration from a single cPanel server.

    Tracks the overall migration job including cPanel server credentials
    and discovered domains.
    """
    STATUS_CHOICES = [
        ('configuring', 'Configuring'),  # User is selecting domains
        ('pending', 'Pending'),           # Waiting for job to start
        ('running', 'Running'),           # Job is processing
        ('completed', 'Completed'),       # All domains done
        ('failed', 'Failed'),             # Job failed
        ('cancelled', 'Cancelled'),       # User cancelled
    ]

    # cPanel Server credentials
    cpanel_hostname = models.CharField(
        max_length=253,
        help_text="cPanel server hostname or IP"
    )
    cpanel_port = models.IntegerField(
        default=2083,
        help_text="cPanel port (usually 2083 for HTTPS)"
    )
    cpanel_username = models.CharField(
        max_length=100,
        help_text="cPanel username"
    )
    cpanel_api_token = models.CharField(
        max_length=500,
        help_text="Encrypted cPanel API token"
    )
    cpanel_verify_ssl = models.BooleanField(
        default=True,
        help_text="Verify SSL certificate when connecting to cPanel"
    )

    # Discovery cache (populated after connecting)
    discovered_domains = models.JSONField(
        default=list,
        help_text="List of discovered domain info from cPanel"
    )
    ...
    job_name = models.CharField(
        max_length=100,
        blank=True,
        help_text="Kubernetes Job name for the migration worker"
    )
    ...
    backup_status = models.CharField(
        ...
        blank=True,
        default='',
        help_text="Current backup download status message"
    )
    backup_progress = models.IntegerField(
        default=0,
        help_text="Backup download progress percentage (0-100)"
    )

    # SFTP receiver info (set when batch starts)
    sftp_port = models.IntegerField(
        null=True,
        blank=True,
        help_text="NodePort for the batch-specific SFTP service"
    )
    sftp_username = models.CharField(
        max_length=50,
        blank=True,
        help_text="SFTP username for backup receiver"
    )
    sftp_password = models.CharField(
        max_length=100,
        blank=True,
        help_text="SFTP password for backup receiver"
    )
    sftp_hostname = models.CharField(
        max_length=255,
        blank=True,
        help_text="Public hostname for SFTP access"
    )
    ...
    def __str__(self):
        return f"Migration from {self.cpanel_hostname} ({self.status})"
    ...
    def completed_domains(self):
        return self.domains.filter(status='completed').count()

    @property
    def failed_domains(self):
        return self.domains.filter(status='failed').count()
    ...
    def progress_percent(self):
        ...
        if total == 0:
            return 0
        completed = self.domains.exclude(status__in=['queued', 'skipped']).count()
    ...
    def set_api_token(self, plaintext_token: str):
        ...
        Args:
            plaintext_token: The plaintext API token to encrypt
        """
        from dashboard.services.encryption import encrypt_token
        if plaintext_token:
            self.cpanel_api_token = encrypt_token(plaintext_token)
        else:
            self.cpanel_api_token = ''
    ...
    def get_api_token(self) -> str:
        ...
        Returns:
            Decrypted API token or empty string if decryption fails
        """
        from dashboard.services.encryption import decrypt_token, is_encrypted
        if not self.cpanel_api_token:
            return ''
        if is_encrypted(self.cpanel_api_token):
            try:
                return decrypt_token(self.cpanel_api_token)
            except ValueError:
                return ''
        # Return as-is if not encrypted (for backwards compatibility)
        return self.cpanel_api_token

    def clear_api_token(self):
        """Clear the API token (for security after migration completes)."""
        self.cpanel_api_token = ''
        self.save(update_fields=['cpanel_api_token'])


class MigrationDomain(models.Model):
    """
    A single domain to migrate within a batch.

    Tracks individual domain migration status, selected database,
    selected mailboxes, and target owner/package.
    """
    STATUS_CHOICES = [
        ('queued', 'Queued'),
        ('fetching', 'Fetching Backup'),
        ('creating', 'Creating Domain'),
        ('importing_files', 'Importing Files'),
        ('importing_db', 'Importing Database'),
        ('importing_mail', 'Importing Mailboxes'),
        ('syncing_mail', 'Syncing Email'),
        ('completed', 'Completed'),
        ('failed', 'Failed'),
        ('skipped', 'Skipped'),
    ]
    ...
    # Source domain info (from cPanel)
    source_domain = models.CharField(
        max_length=253,
        help_text="Domain name on cPanel"
    )
    source_username = models.CharField(
        max_length=100,
        help_text="cPanel account username owning this domain"
    )
    ...
    target_domain = models.ForeignKey(
        ...
        help_text="Created KubePanel domain"
    )
    target_owner = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='migration_domains',
        help_text="Owner for the new domain in KubePanel"
    )
    target_package = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        help_text="Package for the new domain"
    )

    # Database selection (ONE only, or empty to skip)
    available_databases = models.JSONField(
        default=list,
        help_text="List of available databases: [{name, size_mb}, ...]"
    )
    selected_database = models.CharField(
        max_length=100,
        blank=True,
        help_text="Selected database to import (empty = skip)"
    )

    # Mailbox selection (multiple)
    available_mailboxes = models.JSONField(
        default=list,
        help_text="List of available mailboxes: [{email, size_mb}, ...]"
    )
    selected_mailboxes = models.JSONField(
        default=list,
        help_text="List of selected email addresses to import"
    )
    email_method = models.CharField(
        max_length=10,
        choices=EMAIL_METHOD_CHOICES,
        default='backup',
        help_text="How to migrate email"
    )
    ...
    class Meta:
        verbose_name = "Migration Domain"
        verbose_name_plural = "Migration Domains"
        ordering = ['id']  # Process in order added
