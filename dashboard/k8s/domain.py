...
class DomainSpec:
    ...
    def to_dict(self) -> dict:
        ...
        if self.sftp_enabled is not None or self.sftp_type is not None:
            sftp = {}
            if self.sftp_enabled is not None:
                sftp["enabled"] = self.sftp_enabled
            if self.sftp_type is not None:
                sftp["type"] = self.sftp_type
            spec["sftp"] = sftp

        if self.redis_enabled is not None:
...
def get_domain_config(domain_name: str) -> dict:
    ...
    return {
        ...
        # Container options
        "sftp_type": spec.get("sftp", {}).get("type", "standard"),
