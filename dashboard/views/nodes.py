...
def delete_pod(request, namespace, name):
    """Delete a pod with confirmation page."""
    is_super = request.user.is_superuser
    try:
        base, headers, verify = _load_k8s_auth()
    except Exception as e:
    ...
    # Get pod details
    pod_url = f"{base}/api/v1/namespaces/{namespace}/pods/{name}"
    try:
        pod_resp = requests.get(pod_url, headers=headers, verify=verify, timeout=10)
    except Exception as e:
        messages.error(request, f"Error fetching pod details: {e}")
        return redirect('pods_status')

    if pod_resp.status_code == 404:
        messages.error(request, f"Pod '{name}' not found.")
        return redirect('pods_status')

    if not pod_resp.ok:
        messages.error(request, f"Could not fetch pod details (status={pod_resp.status_code})")
        return redirect('pods_status')
    ...
    # CRITICAL: Authorization check - users can only delete their own pods
    if not is_super:
        user_domain_cr_names = [d.replace('.', '-') for d in Domain.objects.filter(owner=request.user).values_list('domain_name', flat=True)]
        if not domain_label:
            messages.error(request, "Permission denied. You can only delete pods belonging to your domains.")
            return redirect('pods_status')
        if domain_label not in user_domain_cr_names:
            messages.error(request, "Permission denied. This pod does not belong to your domain.")
            return redirect('pods_status')

    # Extract pod info for display
    pod_status = pod_json.get('status', {}).get('phase', 'Unknown')
    pod_created = pod_json.get('metadata', {}).get('creationTimestamp', '')
    ...
    if request.method == 'POST':
        # Delete the pod
        try:
            delete_resp = requests.delete(pod_url, headers=headers, verify=verify, timeout=10)
            if delete_resp.status_code in [200, 202]:
                messages.success(request, f"Pod '{name}' deleted successfully. Kubernetes will recreate it if managed by a controller.")
            else:
                messages.error(request, f"Failed to delete pod: {delete_resp.text}")
        except Exception as e:
            messages.error(request, f"Error deleting pod: {e}")
        return redirect('pods_status')

    # GET: Show confirmation page
    return render(request, 'main/delete_pod.html', {
        'pod_name': name,
        'namespace': namespace,
