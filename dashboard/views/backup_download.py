...
class DownloadBackupArchiveView(View):
    ...
    def get(self, request, backup_name):
        ...
        try:
            ...
            # 1. Create manifest.json in /tmp (small file)
            # 2. Decompress database to /tmp/database.sql
            # 3. Create symlink html -> /data to avoid copying large files
            # 4. Stream tar with -h to follow symlinks (archives /data contents as html/)
            tar_cmd = f"""
                cd /tmp && \
                echo '{manifest_json}' > manifest.json && \
                zstd -d {db_backup_path} -o database.sql 2>/dev/null || echo "" > database.sql && \
                ln -sf /data html && \
                tar czhf - manifest.json database.sql html
            """
