...
def list_mail_users(request):
    ...
    # Get package limits for quota display
    pkg = getattr(request.user.profile, 'package', None)
    mail_user_count = mail_users.count()
    max_mail_users = pkg.max_mail_users if pkg else None
    at_limit = max_mail_users is not None and mail_user_count >= max_mail_users

    return render(request, "main/list_mail_users.html", {
        "mail_users": mail_users,
        "mail_user_count": mail_user_count,
        "max_mail_users": max_mail_users,
        "at_limit": at_limit,
    })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if not request.user.is_superuser:
                kind, obj = form.cleaned_data['mail_domain']
                owner = obj.owner if kind == 'domain' else obj.domain.owner
...
def edit_mail_user(request, user_id):
    ...
    if request.method == 'POST':
        form = MailUserForm(request.POST, user=request.user, instance=mail_user)
        if form.is_valid():
            if not request.user.is_superuser:
                kind, obj = form.cleaned_data['mail_domain']
                owner = obj.owner if kind == 'domain' else obj.domain.owner
...
def delete_mail_user(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if imsure == mail_user.email:
            email = mail_user.email
            mail_user.delete()
            messages.success(request, f"Mail user '{email}' deleted successfully.")
            return redirect("list_mail_users")
        else:
            return render(request, "main/delete_mail_user.html", {
                "mail_user": mail_user,
                "error": "Email address does not match. Please type the exact email to confirm."
            })
...
def mail_alias_delete(request, pk):
    ...
    if request.method == 'POST':
        imsure = request.POST.get('imsure', '')
        if imsure == alias.source:
            alias.delete()
            messages.success(request, f"Mail alias '{alias.source}' deleted successfully.")
            return redirect('list_mail_users')
        else:
            return render(request, 'main/mail_alias_confirm_delete.html', {
                'alias': alias,
                'error': 'Email does not match. Please type the exact source email to confirm.'
            })
...
def download_mailbox(request, user_id):
    ...
    domain_name = mail_user.mail_domain_name
