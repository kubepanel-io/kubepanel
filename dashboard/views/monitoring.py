        ...
        if response.status_code == 200:
            ...
            for line in response.text.splitlines():
                if not line.strip():
                    continue
                try:
                    # Convert \xNN hex escapes to \u00NN unicode escapes (JSON-compatible)
                    # nginx logs request_body with \x22 for quotes, which is invalid JSON
                    line = re.sub(r'\\x([0-9a-fA-F]{2})', r'\\u00\1', line)
                    # Convert other invalid escape sequences (like \0, \a, \e) to unicode
                    # Match backslash followed by char that's NOT a valid JSON escape
                    line = re.sub(r'\\([^"\\/bfnrtu])', lambda m: f'\\u00{ord(m.group(1)):02x}', line)
                    ...
                    logger.warning(f"JSON parse failed for ingress log: {e} - Line preview: {line[:300]}...")
