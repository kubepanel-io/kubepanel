...
from dashboard.models import DNSZone, DNSRecord, CloudflareAPIToken, Domain, DomainAlias
...
# =============================================================================
# Alias DNS management (via DNSZone CR, resolved by alias PK)
# =============================================================================

@login_required
def alias_dns_records(request, pk):
    """List DNS records for a domain alias from its DNSZone CR."""
    ...
    try:
        dnszone = get_dnszone(alias_name)
    ...
    return render(request, 'main/alias_dns_records.html', {
        'alias': alias,
        'domain': domain,
...
@login_required
def add_alias_dns_record(request, pk):
    """Add DNS record to a domain alias's DNSZone CR."""
    alias = get_object_or_404(DomainAlias, pk=pk)
    domain = alias.domain
    if domain.owner != request.user and not request.user.is_superuser:
        return render(request, "main/error.html", {"error": "Permission denied."})
    ...
    try:
        get_dnszone(alias_name)
    except K8sNotFoundError:
        messages.error(request, f"DNS is not enabled for {alias_name}. Please enable DNS first.")
        return redirect('view_alias', pk=pk)
    ...
    if request.method == 'POST':
        ...
        try:
            add_dnszone_record(alias_name, new_record)
            messages.success(request, "DNS record added. It may take a moment to propagate to Cloudflare.")
            return redirect('alias_dns_records', pk=pk)
        except K8sClientError as e:
            messages.error(request, f"Failed to add DNS record: {e}")

    return render(request, 'main/add_alias_dns_record.html', {
        'alias': alias,
        'domain': domain,
        'domain_name': alias_name,
        'record_types': ['A', 'AAAA', 'CNAME', 'MX', 'TXT', 'SRV', 'CAA'],
...
@login_required
def edit_alias_dns_record(request, pk, record_index):
    """Edit DNS record in a domain alias's DNSZone CR."""
    alias = get_object_or_404(DomainAlias, pk=pk)
    domain = alias.domain
    if domain.owner != request.user and not request.user.is_superuser:
        return render(request, "main/error.html", {"error": "Permission denied."})
    ...
    try:
        record = get_dnszone_record(alias_name, record_index)
        if record is None:
            messages.error(request, "Record not found.")
            return redirect('alias_dns_records', pk=pk)
    except K8sNotFoundError:
        messages.error(request, f"DNS zone not found for {alias_name}")
        return redirect('view_alias', pk=pk)
    except K8sClientError as e:
        messages.error(request, f"Error fetching DNS zone: {e}")
        return redirect('view_alias', pk=pk)
    ...
    if request.method == 'POST':
        ...
        try:
            update_dnszone_record(alias_name, record_index, updated_record)
            messages.success(request, "DNS record updated. It may take a moment to propagate to Cloudflare.")
            return redirect('alias_dns_records', pk=pk)
        except K8sClientError as e:
            messages.error(request, f"Failed to update DNS record: {e}")

    return render(request, 'main/edit_alias_dns_record.html', {
        'alias': alias,
        'domain': domain,
        'domain_name': alias_name,
        'record': record,
...
@login_required
def delete_alias_dns_record(request, pk, record_index):
    """Delete DNS record from a domain alias's DNSZone CR."""
    alias = get_object_or_404(DomainAlias, pk=pk)
    domain = alias.domain
    if domain.owner != request.user and not request.user.is_superuser:
        return render(request, "main/error.html", {"error": "Permission denied."})
    ...
    if request.method == 'POST':
        try:
            delete_dnszone_record(alias_name, record_index)
            messages.success(request, "DNS record deleted. It may take a moment to be removed from Cloudflare.")
            return redirect('alias_dns_records', pk=pk)
        except K8sClientError as e:
            messages.error(request, f"Failed to delete DNS record: {e}")
            return redirect('alias_dns_records', pk=pk)

    return render(request, 'main/delete_alias_dns_record_confirm.html', {
        'alias': alias,
        'domain': domain,
        'domain_name': alias_name,
        'record': record,
