...
def delete_package(request, pk):
    """Delete a package (superuser only)."""
    if not request.user.is_superuser:
        messages.error(request, "You don't have permission to delete packages.")
        return redirect('list_packages')

    package = get_object_or_404(Package, pk=pk)

    # Check if any users are assigned to this package
    assigned_users = UserProfile.objects.filter(package=package)
    if assigned_users.exists():
        return render(request, 'main/delete_package.html', {
            'package': package,
            'error': f'Cannot delete package. {assigned_users.count()} user(s) are assigned to this package. '
                     'Please reassign them to another package first.'
        })

    if request.method == 'POST':
        imsure = request.POST.get('imsure', '')
        if imsure == package.name:
            ...
            return render(request, 'main/delete_package.html', {
                'package': package,
                'error': 'Package name does not match. Please type the exact package name to confirm.'
            })

    return render(request, 'main/delete_package.html', {'package': package})
